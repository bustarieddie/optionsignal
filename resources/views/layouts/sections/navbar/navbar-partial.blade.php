  ...
  <ul class="navbar-nav flex-row align-items-center ms-md-auto">
    ...
    <li class="nav-item dropdown-notifications navbar-dropdown dropdown me-4 me-xl-1">
      <a class="nav-link dropdown-toggle hide-arrow btn btn-icon btn-text-secondary rounded-pill"
        ...
        @if (auth()->check() && auth()->user()->unreadNotifications()->exists())
      ...
      <ul class="dropdown-menu dropdown-menu-end py-0">
        <li class="dropdown-menu-header border-bottom py-50">
          <div class="dropdown-header d-flex align-items-center py-2">
            ...
            <div class="d-flex align-items-center h6 mb-0">
              @php $ospUnread = auth()->check() ? auth()->user()->unreadNotifications()->count() : 0; @endphp
              @if ($ospUnread)
                <span class="badge rounded-pill bg-label-primary fs-xsmall me-2">{{ $ospUnread }} New</span>
              @endif
            </div>
          </div>
        </li>
        <li class="dropdown-notifications-list scrollable-container">
          <ul class="list-group list-group-flush">
            @forelse (auth()->user()?->notifications()->latest()->take(8)->get() ?? [] as $note)
              <a href="{{ route('notifications.go', $note->id) }}"
                class="list-group-item list-group-item-action dropdown-notifications-item {{ $note->read_at ? 'marked-as-read' : '' }}">
                <div class="d-flex">
                  <div class="flex-shrink-0 me-3">
                    <div class="avatar">
                      <span class="avatar-initial rounded-circle bg-label-{{ ($note->data['signal_type'] ?? '') === 'buy_put' ? 'danger' : 'success' }}">
                        <i class="icon-base ri ri-flashlight-line icon-18px"></i>
                      </span>
                    </div>
                  </div>
                  <div class="flex-grow-1">
                    <h6 class="small mb-1">{{ ($note->data['ticker'] ?? 'Signal') }}
                      <span class="badge bg-label-secondary">{{ $note->data['grade'] ?? '' }}</span></h6>
                    <small class="mb-1 d-block text-body">{{ $note->data['message'] ?? 'New notification' }}</small>
                    <small class="text-body-secondary">{{ $note->created_at->diffForHumans() }}</small>
                  </div>
                  @unless ($note->read_at)
                    <div class="flex-shrink-0"><span class="badge badge-dot bg-primary"></span></div>
                  @endunless
                </div>
              </a>
            @empty
              <li class="list-group-item text-center text-body-secondary py-4">No notifications yet.</li>
            @endforelse
          </ul>
        </li>
        <li class="border-top">
          <div class="d-grid p-4">
            <form method="POST" action="{{ route('notifications.read-all') }}">
              @csrf
              <button type="submit" class="btn btn-primary btn-sm d-flex w-100 justify-content-center">
    ...
    <li class="nav-item navbar-dropdown dropdown-user dropdown">
      ...
      <ul class="dropdown-menu dropdown-menu-end mt-3 py-2">
          ...
          <a class="dropdown-item" href="{{ route('account.profile') }}">
            <div class="d-flex align-items-center">
              ...
              <div class="flex-grow-1">
                ...
                <small class="text-body-secondary">{{ Auth::check() ? Auth::user()->getRoleNames()->first() : '' }}</small>
              </div>
            </div>
          </a>
        </li>
        ...
        <li>
          <a class="dropdown-item" href="{{ route('account.profile') }}">
            <i class="icon-base ri ri-user-3-line icon-22px me-2"></i> <span class="align-middle">My Profile</span> </a>
        </li>
        <li>
          <a class="dropdown-item" href="{{ route('account.api-tokens') }}"> <i
              class="icon-base ri ri-key-2-line icon-22px me-3"></i><span class="align-middle">API Tokens</span>
          </a>
        </li>
        <li>
